...
# # --- 1. USER MANAGEMENT (EXISTING LOGIC) ---
...
# ):
#     """Allows Admin to see every registered account."""
...
#     current_user: models.User = Depends(oauth2.get_current_user)
# ):
#     """Disable or Enable a user account (Ban/Unban)."""
#     if current_user.role != "admin":
#         raise HTTPException(status_code=403, detail="Admin access required")
...
# # --- 2. MANUAL INTERVIEW VERIFICATION FLOW (NEW) ---
...
#     current_user: models.User = Depends(oauth2.get_current_user)
# ):
#     """Fetch profiles of teachers who are not yet verified."""
#     if current_user.role != "admin":
#         raise HTTPException(status_code=403, detail="Admin access required")

#     # Joins TeacherProfile with User to check is_verified status
...
#     current_user: models.User = Depends(oauth2.get_current_user)
# ):
#     """Update teacher status after manual interview (verified/rejected/scheduled)."""
#     if current_user.role != "admin":
#         raise HTTPException(status_code=403, detail="Admin access required")
...
#     # Update the interview status funnel
#     teacher.interview_status = status_data.new_status
    
#     # Only flip the 'is_verified' switch if they officially pass
...
#     current_user: models.User = Depends(oauth2.get_current_user)
# ):
#     """View platform growth and revenue stats."""
#     if current_user.role != "admin":
...
#         "user_breakdown": {
#             # UPDATED: Counts all registered users with the 'student' role instead of just profile entries
...
# --- 1. PUBLIC ANALYTICS (NEW: For Hero Section) ---
