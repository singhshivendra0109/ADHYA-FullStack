...
//         setTutors(data.filter(tutor => tutor.is_verified === true)); 
//         setLoading(false);
//       } catch (err) {
...
//       {/* 🆕 Using max-w-[1600px] to give more room for 5 boxes */}
...
//               spaceBetween={20} // 🆕 Reduced slightly to fit 5 boxes comfortably
//               slidesPerView={1}
//               loop={true} 
//               autoplay={{
//                 delay: 3500,
...
//                 1536: { slidesPerView: 5 } // 🆕 Shows exactly 5 teachers on wide screens
//               }}
...
//                       <p className="text-gray-500 text-xs md:text-sm font-medium line-clamp-3 mb-8 leading-relaxed italic text-left">
...
//             <button className="swiper-button-prev-tutor absolute top-1/2 -left-4 lg:-left-12 z-20 w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-xl border border-emerald-50 text-[#2D8B8B] hover:bg-[#2D8B8B] hover:text-white transition-all -translate-y-1/2 opacity-0 group-hover/swiper:opacity-100">
//               <ChevronLeft size={24} />
//             </button>
//             <button className="swiper-button-next-tutor absolute top-1/2 -right-4 lg:-right-12 z-20 w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-xl border border-emerald-100 text-[#2D8B8B] hover:bg-[#2D8B8B] hover:text-white transition-all -translate-y-1/2 opacity-0 group-hover/swiper:opacity-100">
  ...
  useEffect(() => {
    const getTutors = async () => {
      ...
      try {
        ...
        // 1. Pehle sirf verified tutors ko filter karo
        let verifiedTutors = data.filter(tutor => tutor.is_verified === true);
        
        // 2. 🆕 FRONTEND SEARCH FILTER LOGIC ADDED HERE
        if (searchQuery) {
        ...
        // 3. Frontend Subject Filter (Safety backup in case backend misses it)
        if (subjectFilter) {
          verifiedTutors = verifiedTutors.filter(tutor => 
